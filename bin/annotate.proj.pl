...
python /srv/data2/software/moroz-python/zeroclick/scripts/systemtools.py
--add-project --assign-workflow --add-jobs adapter_trim quality_trim
read_normalization assemble quantification blast_nr blast_swissprot
pfam kegg go --set-config adapter_trim:+ quality_trim:+
read_normalization:+ assemble:+ quantification:+ blast_nr:+
blast_swissprot:+ pfam:+ kegg:+ go:+ --set-args
"quality_trim|pipeline_args;--paired-end"
"adapter_trim|pipeline_args;--paired-end"
"read_normalization|pipeline_args;--paired-end"
"assemble|pipeline_args;--paired-end" --project-names
