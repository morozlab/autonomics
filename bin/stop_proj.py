...
def main():
    ...
    if row:
       ...
       sys.exit()
    u = jn_mapping_table.update().where(jn_mapping_table.c.project_id==project_id).values(started = 'Y', finished = 'Y', queued = 'Y').execute()
    u = quenew_table.delete().where(quenew_table.c.project_id==project_id).execute()
    print "successfully stopped project: ", project_name
