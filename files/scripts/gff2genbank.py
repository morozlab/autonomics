...
"""
    for rec in fasta_iter:
        if len(rec.name) > 16 and rec.name.find("|") > 0:
            ...
            rec.id = new_id
            rec.name = new_id
        elif(len(rec.name) > 16 and rec.name.find(".") > 0):
            new_id = rec.name.split(".")[1]
            if(len(new_id) > 16 and new_id.find("_") > 0): new_id = new_id.split("_")[1];

            print "Warning: shortening name %s to %s" % (rec.id, new_id)
            rec.id = new_id
            rec.name = new_id
