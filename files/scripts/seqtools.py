...
parser.add_argument('--mapping-accuracy', dest='mapping_acc', default=False, const=True, action='store_const', help='Calculate the mapping accuracy of short reads to a target genome. Requires an alignment file and a FASTA-formatted genome file to be specified.')
...
def determine_accuracy_sam(genome_file, sam_aln):
    ...
    genome_dict = SeqIO.index(fh, 'fasta')
    fh.close()
    ...
    for r in sam_file.iterrecords():
        ref = r.reference
        #if this read couldn't be mapped, skip it
        if(ref == "*"):
            continue

        cig_elements = matcher.findall(r.cigar)
        ref_seq = str(genome_dict[ref].seq)
...
def mapping_accuracy(genome_file, aln_file, aln_type='sam'):
    ...
    if(aln_type=='sam'):
        determine_accuracy_sam(genome_file, aln_file)
