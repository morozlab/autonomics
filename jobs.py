...
class Resources:
    ...
    def give_to(self, job):
        '''
            ...
            resources to give, removes the resources from this object and
            returns True.
        '''
...
class Job:
    ''' This class is the ancestor of all analysis tasks.
        ...
        As such, it has the following attributes:
            ...
            - local_dir (str):
                ...
                Defaults to: settings.home_dir + pn
    ...
    def __init__(self, pid, jid, t, executable="NA", resources="",
        ...
        self.local_dir = settings.home_dir + "/" + self.pn + "/"
        if self.special_run:
            self.local_dir = settings.home_dir + "/special_runs/" + \
                            self.job_name + "/"
        ...
        while (retries > 0):
            try:
                ...
                netutils.SSHConnection(settings.creds['128.227.123.35'].host,
                              ...
                              port=2122)
                print "connected to 128.227.123.35"
                break
            except Exception:
...
class AssemblyProcess(PipeProcess):
    ...
    def run(self):
        ...
        out_dir = settings.home_dir + self.pn + "/"
        if (self.paired_end):
...
class BlastAssociationProcess(PipeProcess):
    ...
    def __init__(
        ...
        self.local_dir = settings.home_dir + "/" + base_name + "/"
...
class QuantificationJob(HPCJob):
    ...
    def _handle_args(self):
        ...
        if('blast' in aligner):
            ...
            #        if(not hasattr(self.process_args, "db")):
            #    self.process_args.db = "<db>"
            #if(not hasattr(self.process_args, "query")):
            #    self.process_args.query = "<query>"


            #if(not os.path.isfile(self.process_args.db)):
            #if(self.quant_type != BLAST_TYPE):
            #    self.db_file = self.pn + "_project.fasta"
            #else:
            #    self.db_file = self.pn + ".fasta"
                #add code to check if fastq file exists, and if so convert

                #self.local_db_path = self.local_dir + self.db_file

                #else:
                #self.local_db_path = self.process_args.db
                #discard, self.db_file = os.path.split(self.local_db_path)
    ...
    def move_db_files(self):
        '''
            ...
            computational resource, after remaning them to their full remote
            path.
        '''
    ...
    def prep_blast(self):
        ...
        #add a directive to make the wu-blast database
        #self.hpc_command = "".join(["xdformat ", file_io.wu_blast_typeflags[self.db_type],
        #                                " -o ", self.remote_db_path, " ",
        #                                self.remote_db_path, "\n"])

        if(self.db_type == "NT"):
            exts = ['.nin', '.nsq', '.nhr']
            #self.db_files.append(self.local_db_path + ".nin")
            #self.db_files.append(self.local_db_path + ".nsq")
            #self.db_files.append(self.local_db_path + ".nhr")
            # blast_files = [(self.db_file + ".nin"), (self.db_file + ".nsq"), + (self.db_file + ".nhr")]
        #    self.db_files.extend(blast_files)
        else:
            exts = ['.pin', '.psq', '.phr']
...
class Qsub:
    ...
    def retrieve_output(self, c):
        ...
        try:
            ...
            sys.stdout.write("Error " + str(e.errno) + " retrieving: " +\
                              self.remote_dir + "/" + self.current_output  +\
                               "\n")
            return None
    ...
    def submit(self, c):
        ...
        c.put(self.local_dir + self.name + ".qsub",
              self.remote_dir + "/" + self.name + ".qsub")
...
class HPCProcess(PipeProcess):
    ...
    def _mail_connect(self):
        ...
        while(True and self.mail_enabled):
            try:
                ...
                if(retries > 0):
                    sys.stderr.write("Exception in _mail_connect().\n")
    ...
    def check_individual_job(self, job_index, c):
        ...
        command =  "qstat -u mcitar | grep -c " + proc.job_id[:16]
        try:
            ...
            sys.stderr.write("Error retrieving status of running job " +\
                              proc.name + " - abandoning.\n")
            running = -1
