	...
	public class SequencePanel extends MovieClip{
		...
		private function doLoad(res:Array){
			...
			for(var i = 0; i < res.length; i++){
				...
				addSequence(res[i]['seq_id'], res[i]['project_id'], res[i]['description'], res[i]['type']);
			}
		...
		private function addSequence(ID:Number, project:Number, description:String, type:String){
			var seq:Sequence = new Sequence(ID, project, description, this, type);
			seq.y = lastY + 10;
			seq.initYs();
			lastY = lastY + 10 + seq.height;
		...
		private function addDescriptionText(str:String, type:String){
			...
			descriptionText.y = container.y - descriptionText.height - 10;
