...
if($found == 0){
  $tableStr = "CREATE TABLE " . $fullname . " (seq_id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, sb_id INT(20), project_id INT, file_id INT, NT_sequence TEXT, AA_sequence TEXT,  description TEXT, type char(2), length INT, date DATE)ENGINE=innodb";
  $dbh->do($tableStr);
}
