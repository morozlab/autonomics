			...
			if(isset($search)){
				...
				if($sbIDSearch == false){
					...
					if($haveAnnotation == false){
						...
						$query = "SELECT sorted_homology.annotation_id, sb_id, evalue, text, annotation_db.source, abundance FROM annotation_db, sorted_homology WHERE (MATCH(text) AGAINST ('" . $search . "' IN BOOLEAN MODE)) AND sorted_homology.annotation_id=annotation_db.annotation_id AND project_id='" . $projectID . "' and sort_id='1' ORDER BY ranking";
						$result = mysql_query($query);
						$resultArray = generateResultArray($result);
	...
	if($numSeqs == 0){
		...
		//case when the project has annotation
			...
			//need to look up annotation information for each sequence
				...
				for($i = $pageOffset; $i < $lastIndex; $i++){
					$query = "SELECT annotation_id, text, evalue, abundance, source FROM sorted_homology WHERE project_id='" . $projectID . "' AND sort_id='1' AND sb_id='" . $result[$i]['sb_id'] . "' ORDER BY ranking LIMIT 0,1";
					$result = mysql_query($query);
					$row = mysql_fetch_array($result);
...
function displayPageNav($mode){
	...
	echo("<div id='resultForm'><form name='pageSelect' action='viewSeqs_new.php' method='get'>");
...
function displayAddlSearch(){
	...
	echo("<div id='addlSearch'><div style='float:left;'><form name='addlSearch' id='addlSearch' action='viewSeqs_new.php' method='get'>");
