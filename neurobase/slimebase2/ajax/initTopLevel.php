...
foreach($array as $compartment){
	if($compartment == "P"){
		$fullName = "Biological Process";
	}
	else if($compartment == "C"){
		$fullName = "Cellular Component";
	}
	else if($compartment == "F"){
		$fullName = "Molecular Function";
	}
	$count = 0;
	...
	*/
	...
	$count += addToCount($query);
	$resultString = $resultString . "<div style='padding-bottom: 4px; width:880px; float:left;' id='" . $compartment . "_div'" . '><a id="' . $compartment . '" target="_self" href="#" onclick="toggleView(this.id, \'expand\', \'top\', ' . $count . ');return(false);">' . $fullName . ' (' . $count . ')</a><div id="' . $compartment . "_children" . '"></div></div>';
	$index++;
}
}
else{
