...
if($method == 'updateCart'){
	...
	$echoArray[0] .= "<div class='extraContentNavElement' id='cross-link' onclick='setActiveFeature(this.id);return(false);'><a href='#'>Homology Search</a></div>";
	$echoArray[1] .= "<div id='cross-comparison-selects'><form id='comparison-form' name='comparison-form' name='comparison-form'>compare <select name='comparison-select-1' id='comparison-select-1' class='comparison-select' onchange='reloadSecondSelect();'>";
	$echoArray[1] .= $optionList1;
	$echoArray[1] .= "</select> to <span id='comparison-select-2-span'><select name='comparison-select-2' id='comparison-select-2' class='comparison-select' onchange='fetchCrossComparison(1)'>";
	$echoArray[1] .= $optionList2; 
	$echoArray[1] .= "</select></span></form></div>";	
	$echoArray[4] = "<div class='extraContentNavElement'><a href='viewSeqs.php?projectID=" . $projectID . "'>Find Sequences</a></div>";
	//check if there is GO annotation
	$query = "SELECT sb_id FROM go_annotation WHERE project_id ='" . $projectID . "' LIMIT 0,1";
	$result = mysql_query($query);
	if(mysql_num_rows($result)){
		$echoArray[4] .= "<div class='extraContentNavElement'><a href='go/GOBrowser.php?projectID=" . $projectID . "'>Gene Ontology</a></div>";
	}
	
	$echoArray[2] .= "<div class='extraContentNavElement' id='information-link' onclick='setActiveFeature(this.id);return(false);'><a href='#'>Information</a></div>";
	$echoArray[3] .= "<div id='information-container'><div class='cross-comparison-header'>This tab contains helpful information such as contact details and organism descriptions.</div><div id='information-content'>";
	$query = "SELECT long_info, latin_name FROM project_information WHERE project_id='" . $projectID . "'";
	$result = mysql_query($query);
	$row = mysql_fetch_array($result);
	$echoArray[3] .= "<h1>About " . $row['latin_name'] . "</h1>";
	$echoArray[3] .= "<p>" . $row['long_info'] . "</p>";
	...
	if(mysql_num_rows($result)){
		$row = mysql_fetch_array($result);
		$echoArray[3] .= "<h1>Abstract</h1><span class='abstractTitle' id='abstractTitle'>" . $row['title'] . "</span><p class='abstractAuthors' id='abstractAuthors'>" . $row['authors'] . "</p><p class='abstractText' id='abstractText'>" . $row['text'] . "</p>"; 
	}
	$address = "9505 Oceanshore Blvd<br/>Saint Augustine, FL 32080<br/>";
	$echoArray[3] .= "<h1>NeuroBase was designed and constructed at the University of Florida</h1>";
	$echoArray[3] .= "<div class='people-details'>Professor Leonid L. Moroz<br/>Princple Investigator<br/>" . $address . "904-461-4020<br/><a href='mailto:moroz@whitney.ufl.edu'>moroz@whitney.ufl.edu</a></div>";
	$echoArray[3] .= "<div class='people-details'>Mathew Citarella<br/>Bioinformatics/Design<br/>" . $address . "904-461-4007<br/><a href='mailto:mathew.citarella@gmail.com'>mathew.citarella@gmail.com</a></div>";
	$echoArray[3] .= "<div class='people-details'>Dr. Andrea Kohn<br/>Genomics/Molecular Biology<br/>" . $address . "904-461-4007<br/><a href='mailto:abkohn@msn.com'>abkohn@msn.com</a></div>";
	$echoArray[3] .= "<div class='contact-message'><p>Please direct comments or questions about the database to <a href='mailto:morozgenomics@whitney.ufl.edu'>morozgenomics@whitney.ufl.edu</a>.</p></div>";
	$echoArray[3] .= "</div></div>";
	echo json_encode($echoArray);
}
