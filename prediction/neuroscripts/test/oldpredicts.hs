...
main = do
 ...
 sequence_ $ [runSignalP,runTargetP,runTMHMM,mergeAllPreds] <*> [fileNames]
...
--------------------------
--merger
merge p1 p2 p3 = (filter ((==3) . length)) . (map sortPreds) . groupByName $ p1 ++ p2 ++ p3
...
mergeAllPreds parsers fileNames = do
 mapM (\parser -> foldM (\preds fileName -> (parser fileName):preds) fileNames) parsers
 parsers <*>
--FIN
