[ENVIRONMENT('dep_dec.pen')]

module dep_dec;    {implementation dependent parts - DECpascal version}
                   { *** has to be linked to libc ***}
...
type string2   = varying[2]   of char;
     ...
     envstring = packed array[1..255] of char;
     strptr    = ^envstring;
...
function getenv(name : envstring) : strptr; external;

function get_logical(logical : string255) : string255;
var    envname  : string255;
begin
  envname :=logical;
  envvalue:=getenv(envname+chr(0));
  if envvalue=nil then get_logical:='' else
  get_logical:=substr(envvalue^,1,index(envvalue^,chr(0))-1);
end;
...
var cl,buffer : string255;
    i         : integer;
begin
  cl:='';
  if argc>0 then for i:=1 to argc do begin
    argv(i,buffer);
    cl:=cl+' '+buffer;
  end;
  get_command_line:=cl;
...
begin
  ...
  if (acc='w') or (acc='W') then
    ...
    begin
      ...
      reset(f);
      if not eof(f) then 
        repeat readln(f,line) until ((length(line)>0) or eof(f));
      reset(f);
      if eof(f) then uxmode:=true else
